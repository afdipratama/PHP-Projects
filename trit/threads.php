...
function requestToken($username, $password, $uuid)
{
    $headers = array();
...
function likeTimeline($token, $uuid)
{
    ...
    while ($likeCount < $maxLikeCount) {
        $headers[] = 'Authorization: Bearer ' . $token; // add header authorization dengan token
        ...
        if (isset($parsegetFeeds['message']) && $parsegetFeeds['message'] == "login_required") {
            echo "TOKEN KAMU EXPIRED, MEMINTA TOKEN BARU...\n";
            $token = requestToken();
            if (!$token) {
                echo 'GAGAL GET AUTH, ULANGI..';
                sleep(10);
            ...
            $headers[6] = 'Authorization: Bearer ' . $token; // Update header authorization dengan token baru
            continue; 
        }

        if (isset($parsegetFeeds['items']) && is_array($parsegetFeeds['items'])) {
            foreach ($parsegetFeeds['items'] as $postingan) {
                ...
                if (isset($parseGaslike['status'])) {
                    if ($parseGaslike['status'] == "ok") {
                        ...
                        $likeCount++; // Tambah jumlah like
                        sleep(17); // Delay selama 3 detik
...
$akunIG = [
    ['username' => 'MASUKAN USERNAME IG KAMU', 'password' => 'MASUKAN PASSWORD KAMU']
];
...
$index = 0;
$akunCount = count($akunIG);

while (true) {
    $username = $akunIG[$index]['username'];
    $password = $akunIG[$index]['password'];

    $token = requestToken($username, $password, $uuid);
    ...
    try {
        ...
        echo $e->getMessage() . ", Mengambil Token Baru...\n";
        continue;
    }
